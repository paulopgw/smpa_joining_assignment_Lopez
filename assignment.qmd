...
combined_df <- bind_rows(primary_dem, primary_rep)




```
Here i am creating a new combined dataframe under the name combined_df 
...
combined_df <- combined_df %>% clean_names()
...
```{r}
  ...
  by = c("precinct" = "precinct", "county" = "county")


```

Here I am following the instructions, doing a left join. Here I am including all the information from the first table and joining it with information from the second table that matches.
...
Here I used the mutate function to create a new column, naming it total votes, then adding all the values i wanted in the total votes column.
```
...
governor_totals <- combined_df %>%
  group_by(county_name, election_district_precinct) %>%
  summarize(total_votes = sum(total_votes)) %>%
  arrange(desc(total_votes))

print(governor_totals)

```

What is the name and voting location of the precinct that saw the most votes in the governor's race? Prince George's County, Precinct 15005
...
How many precincts are there, and which county has the most? 12 Precincts. The county that has the most is Prince George's County.

What else would you need to know about these precincts to know whether this might be worth a story? A common variable between them, like demographics.
...
Question:
"In which county did the Republican candidate receive the highest number of votes in the Governor's race, and how many votes did they receive?"

To answer this question I would use the governor_totals dataframe and look for highest number of votes using "desc" to put in descending order.
